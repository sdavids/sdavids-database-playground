...
[WARNING]
====
Exercise caution to avoid adding newlines to the credential files.

This can occur inadvertently if the editor you are using automatically inserts a newline at the end of the file.

To eliminate the newline character at the end of a file, one can utilize the following command:
...
+
As superuser (<<passwords,password>>):
+
[,console]
----
...
[#passwords]
The passwords will be randomly created by the link:scripts/credentials_add.sh[credentials_add] script and stored in the following files in the `.docker/secrets` directory:
...
[TIP]
====
Use the following commands to copy a password to your clipboard, e.g. the superuser:

.macOS
[,console]
----
$ cat .docker/secrets/postgres-superuser-pw | pbcopy
----

.Linux
[,console]
----
$ cat .docker/secrets/postgres-superuser-pw | wl-copy
$ cat .docker/secrets/postgres-superuser-pw | xclip -selection clipboard
----
====
...
host.docker.internal:5432:sd_example:SUPERUSER:PASSWORD
host.docker.internal:5432:sd_example:sd_admin:PASSWORD
host.docker.internal:5432:sd_example:sd_example_ow:PASSWORD
host.docker.internal:5432:sd_example:sd_example_ro:PASSWORD
host.docker.internal:5432:sd_example:sd_example_rw:PASSWORD
----

Change the file's permissions to `600`:
...
Replace `SUPERUSER` s with the value found in `.docker/secrets/postgres-superuser`.

Replace the `PASSWORD` s with their values found in `.docker/secrets/*-pw`.

Important: The file has to be named `.pgpass` and the permissions have to be `600`!

Verify that passwordless login works:

[,console]
----
$ scripts/docker_psql.sh -p .pgpass -u sd_admin
